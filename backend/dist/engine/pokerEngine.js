...
export const RANK_NAMES_SINGULAR = {
...
export class PokerEngine {
    ...
    startGame(numPlayers) {
        ...
        for (let j = 0; j < 2; j++) {
            for (let i = 0; i < numPlayers; i++) {
                ...
                if (card !== undefined)
                    this.players[i].holeCards.push(card);
            }
        }
    }
    dealFlop() {
    ...
    evaluate5Cards(cards) {
        ...
        if (isStraight) {
            ...
            for (const val of sortedUnique) {
                for (let i = 0; i < (counts[val] ?? 0); i++)
                    tieBreakerValues.push(val);
            }
        }
        let rank = 1;
        let handName = '';
    ...
    evaluatePlayer(player) {
        ...
        let best = { rank: 0, score: -1, name: '', cards: [] };
